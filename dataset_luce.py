...
npzfile = np.load('xyz_tem_data20070203afternoon501.npz')
tem_mat = npzfile['tem_mat']
xyz_mat = npzfile['xyz_mat']

xy_rel_mat = np.empty_like(xyz_mat[:,0:2])  # define an empty array like (.) with the same shape and dtype 
xy_rel_mat[:,0] = xyz_mat[:,0] - 532975
xy_rel_mat[:,1] = xyz_mat[:,1] - 152200
